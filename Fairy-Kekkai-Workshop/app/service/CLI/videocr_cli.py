...
def main():
    ...
    parser.add_argument(
        "--time_start", type=str, default="0:00", help="Start time (default: 0:00)"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.09,
        help="Maximum time gap in seconds to merge similar subtitles (default: 0.09)",
    )
    parser.add_argument(
        "--use_fullframe",
        type=lambda x: x.lower() == "true",
        default=False,
        help="Use full frame for OCR (true/false)",
    )
    parser.add_argument(
        "--use_gpu",
        type=lambda x: x.lower() == "true",
        default=False,
        help="Enable GPU usage (true/false)",
    )
    parser.add_argument(
        "--use_angle_cls",
        type=lambda x: x.lower() == "true",
        default=False,
        help="Enable Classification (true/false)",
    )
    parser.add_argument(
        "--use_server_model",
        type=lambda x: x.lower() == "true",
        default=False,
        help="Enable usage of server model (true/false)",
    )
    ...
    parser.add_argument(
        "--frames_to_skip", type=int, default=1, help="Frames to skip (default: 1)"
    )
    parser.add_argument(
        "--post_processing",
        type=lambda x: x.lower() == "true",
        default=False,
        help="Enable post processing of subtitles (true/false)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=lambda x: x.lower() == "true",
        default=False,
        help="Enable dual zone OCR processing (true/false)",
    )
    parser.add_argument(
    ...
    if not args.use_fullframe:
        zone1_defined = all(
            v is not None
            for v in [args.crop_x, args.crop_y, args.crop_width, args.crop_height]
        )
        if zone1_defined:
            crop_zones.append(
                {
