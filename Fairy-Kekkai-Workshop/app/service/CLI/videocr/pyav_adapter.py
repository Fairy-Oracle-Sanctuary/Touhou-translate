    ...
    with av.open(path) as container:
        ...
        if is_vfr:
            ...
            if time_end:
                ...
                absolute_target_ms = (
                    relative_end_ms + properties["start_time_offset_ms"]
                )
            ...
            if num_frames_to_map > 0:
                for i in range(num_frames_to_map):
                    print(
                        f"\rMapping frame {i + 1} of {num_frames_to_map}",
                        end="",
                        flush=True,
                    )
                    try:
                ...
                print(
                    "Frame count not found. Estimating progress based on duration...",
                    flush=True,
                )
                ...
                estimated_frames = (
                    int(duration_sec * properties["fps"])
                    if duration_sec > 0 and properties["fps"] > 0
                    else 0
                )
                progress_total = (
                ...
                while True:
                    print(
                        f"\rMapping frame {i + 1} of {progress_total}",
                        end="",
                        flush=True,
                    )
                    try:
