...
class Video:
    ...
    def __init__(
        ...
        initial_num_frames = (
            int(video_track.frame_count) if video_track.frame_count else 0
        )
    ...
    def run_ocr(
        ...
        if self.is_vfr:
            if time_start:
                start_target_ms = (
                    utils.get_ms_from_time_str(time_start) + self.start_time_offset_ms
                )
                ocr_start = utils.get_frame_index_from_ms(
                    self.frame_timestamps, start_target_ms
                )
            else:
                ocr_start = 0
            ...
            if time_end:
                end_target_ms = (
                    utils.get_ms_from_time_str(time_end) + self.start_time_offset_ms
                )
                ocr_end = utils.get_frame_index_from_ms(
                    self.frame_timestamps, end_target_ms
                )
            else:
            ...
            ocr_end = (
                utils.get_frame_index(time_end, self.fps)
                if time_end
        ...
        with Capture(self.path) as v:
            ...
            for i in range(num_ocr_frames):
                print(
                    f"\rStep 1: Processing image {i + 1} of {num_ocr_frames}",
                    end="",
                    flush=True,
                )
                if i % modulo == 0:
                    ...
                    for item in images_to_process:
                        ...
                        if ssim_threshold < 1:
                            ...
                            if prev_samples[zone_idx] is not None:
                                score = fast_ssim.ssim(
                                    prev_samples[zone_idx], sample, data_range=255
                                )
                        ...
                        frame_filename = (
                            f"frame_{frame_index:0{padding}d}_zone{zone_idx}.jpg"
                        )
        ...
        stderr_thread = threading.Thread(
            target=utils.read_pipe, args=(process.stderr, stderr_lines)
        )
        stderr_thread.start()
            ...
            for line in iter(process.stdout.readline, ""):
                ...
                if "ppocr INFO: **********" in line:
                    ...
                    if match:
                        ...
                        ocr_image_index += 1
                        print(
                            f"\rStep 2: Performing OCR on image {ocr_image_index} of {total_images}",
                            end="",
                            flush=True,
                        )
                    ...
                    try:
                        ...
                        if match:
                            ...
                            if self.lang in ARABIC_LANGS:
                                ...
                                corrected_data = [
                                    box,
                                    (utils.convert_visual_to_logical(text), score),
                                ]
        ...
        if exit_code != 0:
            ...
            print(
                f"Error: PaddleOCR failed. See the log file for technical details:\n{log_file_path}",
                flush=True,
            )
            sys.exit(1)
        ...
        for zone_idx in frame_predictions_by_zone:
            frames = sorted(
                frame_predictions_by_zone[zone_idx].values(),
    ...
    def _get_srt_timestamps(self, sub: PredictedSubtitle) -> tuple[str, str]:
        if self.is_vfr:
            ...
            start_time = utils.get_srt_timestamp(
                sub.index_start, self.fps, self.start_time_offset_ms
            )
            end_time = utils.get_srt_timestamp(
                sub.index_end + 1, self.fps, self.start_time_offset_ms
            )
            return start_time, end_time
    ...
    def _get_subtitle_ms_times(self, sub: PredictedSubtitle) -> tuple[float, float]:
        ...
        if end_time_ms is None:
            end_time_ms = self.frame_timestamps.get(sub.index_end, 0) + (
                1000 / self.fps
            )
