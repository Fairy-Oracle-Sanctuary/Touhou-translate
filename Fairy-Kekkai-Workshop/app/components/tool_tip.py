...
class ToolTipPositionManager:
    ...
    def make(position: ToolTipPosition):
        ...
        if str(position) not in managers:
            raise ValueError(f"`{position}` is an invalid tooltip position.")

        return managers[str(position)]()
...
class TableItemToolTipManager(ItemViewToolTipManager):
    ...
    def _pos(self, tooltip: ToolTip, view: QTableView) -> QPoint:
        ...
        y = (
            pos.y()
            - tooltip.height()
            + view.horizontalHeader().isVisible() * view.horizontalHeader().height()
            + 10
        )
        return QPoint(x, y)
...
class ItemViewToolTip(ToolTip):
    ...
    def adjustPos(
        self, view: QAbstractItemView, itemRect: QRect, tooltipType: ItemViewToolTipType
    ):
...
class ItemViewToolTipDelegate(ToolTipFilter):
    ...
    def __init__(
        ...
        showDelay=300,
        tooltipType=ItemViewToolTipType.TABLE,
    ):
...
def setToolTipInfo(
    widget: QWidget, info: str, time: int, position=ToolTipPosition.TOP_LEFT
):
    """设置工具提示信息"""
...
def setToolTipInfos(
    ...
    infos: List[str],
    time: Union[List[int], int],
    position: Union[List[ToolTipPosition], ToolTipPosition] = ToolTipPosition.TOP_LEFT,
):
    """设置多个工具提示信息"""
    time = [time for _ in range(len(widgets))] if type(time) is int else time
    position = (
        [position for _ in range(len(widgets))]
